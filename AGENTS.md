...
code was changed this session beyond AGENTS.md itself — every finding
...
- **A D:-drive redirect via a directory junction was attempted and correctly blocked by the harness**: pointing `.Codex/worktrees` at a junction resolving onto D: (which had ~300GB free) would have worked at the filesystem level (verified with a manual write-through-junction test) but the agent-spawning tool explicitly refuses to create a worktree through a symlinked `.Codex/worktrees` path, as a real security policy against a repo-committed symlink redirecting worktree writes outside the repo boundary. Reverted; all worktrees stayed on C: for the rest of the session, managed via the merge-then-prune wave discipline above.
- **A real `git add -A` mistake was caught and fixed on the spot**: one merge commit accidentally added an active worktree directory as an embedded git repo (git's own warning caught it immediately — `git rm --cached` fixed the index, `.Codex/worktrees/` was added to `.gitignore` so it can never happen again). No data was lost; the fix was folded into the same merge commit via `--amend` before it could propagate.
...
sees it before touching this code, not just this AGENTS.md entry.
...
1. **The real error is now logged.** `_ensureEngine()`'s catch block calls
   ...
   rule doesn't apply to it (AGENTS.md's own carve-out), and it adds no
   visible UI noise.
...
only `v.pain` and `v._cons` across ticks — AGENTS.md's own F0 status
...
- **Chapter 6 (AEMT School, "Going Advanced," design doc §1.6.7/§1.6.8.1) —
  ...
  **Left undone, queued for whoever picks this back up:**
  - **No in-browser playtest at all.** There is currently no way to reach
    ...
    AGENTS.md's own standing rule ("say so explicitly rather than claiming
...
5. **A dead-code sweep is overdue, and it is cheap — STANDING, open.**
   ...
   **RESOLVED (this session) — all four drugs this item originally named.**
   ...
   mechanism — AGENTS.md itself had drifted stale on this one; see lesson 16's
